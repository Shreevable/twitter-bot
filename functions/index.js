...
    } catch (err) {
      functions.logger.warn("Access token may have expired, attempting refresh...", err);
      const {
...
      functions.logger.info(`Successfully refreshed token and fetched tweet: ${tweetId}`);
    }

    const tweetText = tweet.data.text;
    functions.logger.info(`Tweet text for summarization: "${tweetText.substring(0, 100)}..."`); // Log first 100 chars
...
    functions.logger.info(`Generated summary: "${summary.substring(0, 100)}..."`); // Log first 100 chars of summary
...
    functions.logger.info(`Murf.ai response content-type: ${response.headers.get('Content-Type')}`);


    if (!response.ok) {
      const errorText = await response.text();
      functions.logger.error("Murf stream failed:", response.status, errorText);
      return res.status(500).json({ error: `Murf stream failed: ${errorText.substring(0, 200)}` });
    }
